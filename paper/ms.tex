...
vary with the experimental design. Reference priors {\bf cite Bernardo}
also make use of the mutual information but in a different way.
...
version of NS that produces unbiased estimates of the evidence.
...
$\x$ has probability density $f(\x)$, so the differential entropy is
\begin{align}
H &= -\int_{-\infty}^{\infty} f(\x) \log f(\x) \, d\x
\end{align}

What the algorithm actually estimates is
\begin{align}
h &= \int f(\x) \int f(\x')
            \log \left[ P(d(\x', \x) < L | \x) \right]
                        \, d\x' \, d\x.
...
entropy, we can use the fact that density equals mass divided
...
In a one-dimensional parameter space, the algorithm will calculate
\begin{align}
h(L) &= \int f(\x) \int f(\x')
             \log \left[ \int_{x - L}^x f(x') \right]
                        \, d\x' \, d\x.
\end{align}
See the Appendix for a discussion of this quantity. To obtain the differential
entropy, compute $h(L)$ for small $L$ and then add $\log(L)$.
