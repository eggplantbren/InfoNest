...
from 1 to 1 billion. If we learned that the last digit of $\x$ was a 9, we
will have ruled out nine tenths of the possibilities, and therefore obtained
a lot of information about the central issue. However, in many applications
this ``information'' would be basically useless.

The reason is that we often want to ask a slightly different question.
For example, suppose we want to know the value of $\x$ to within a tolerance
of $\pm 5$. The following statements would resolve the issue:
\begin{itemize}
\item $\x \in \{1, ..., 10\}$ and anything that implies it
\item $\x \in \{2, ..., 11\}$ and anything that implies it
...
\end{itemize}
