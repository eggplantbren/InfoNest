...
\caption{Calculates $H(\x) = -\int p(\x) \log p(\x) \, d\x$
         ...
         estimates is $\int f(\x) \int f(\xref)
            \log \left[ P(d(\xref, \x) < L | \x) \right]
                        \, d\xref \, d\x$, the expected value of the
...
I(x; y) &= H(y) - H(y|x) \\
I(x; y) &= H(x) - H(x|y) \\
I(x; y) &= \int p(y) \, D_{\rm KL}\left(p(x|y) \,||\, p(x)\right) \, dy.
\end{align}
In a Bayesian problem where $x$ is an unknown parameter and $y$ is data, the
first and second formulas would be very costly to compute, because of the
high dimensional probability distribution for the dataset. 
