...
runs. The {\todo interim} results are:
\begin{align}
H_{\tau}    &= 7.307 \pm 0.027 \\
H_{\d}      &= 905.5 \pm 5.6   \\
H_{\tau,\d} &= 883.6 \pm 5.2
\end{align}

To conver these to differential entropies, I applied the log-volume
correction factors for 1, 101, and 102 dimensions, which are
$-6.215$, $-790.32$, and $-798.62$ respectively. This gives
the following {\todo interim} values for the differential entropies:
\begin{align}
H'_{\tau}     &= 1.092 \pm 0.027 \label{eqn:h_tau}\\
H'_{\d}       &= 115.2 \pm 5.6   \\
H'_{\tau, \d} &=  93.3 \pm 5.2   \label{eqn:h_joint}
\end{align}
...
The resulting {\todo provisional} mutual information is
$I_{\tau, \d} = 31.7 \pm 3.8$ nats.
...
An example signal observed with the even and uneven observing schedules
...
However, it is worth reflecting on the difficulty of the calculation that
has been done.
...
I_{\x, \d} &= \iint p(\x, \d)
                        ...
                        \, d\x \, d\d
\end{align}
and this measures the dependence between $\x$ and $\d$. A Monte Carlo
...
\end{align}
for each possible data set. Computing $p(\d)$ for even a single
