...
\begin{align}
\depth_{\boldsymbol{x}} &= 698.25 \pm 0.34.
\end{align}
...
To carry out the sampling for this problem, Metropolis-Hastings moves that
explore the joint prior distribution $p(A, \tau, \phi, \boldsymbol{d})$
had to be
...
$A$, $\tau$ or $\phi$ while leaving $\boldsymbol{x}$ fixed
...
$\x = \{x_1, x_2, ..., x_n\}$ is a Pareto distribution, conditional on
a minimum value $x_{\rm min}=1$ and a slope $\alpha$:
\begin{align}
p(\x | \alpha) &= \prod_{i=1}^n \frac{1}{x_i^{\alpha + 1}}
\end{align}
...
observations from past ones, for two reasons. First, the observations may not
provide much information about the parameters (in this case, $\alpha$), and
second, even conditional on the parameters, the predictive distribution for
future data can be heavy-tailed, implying a lot of uncertainty. However, there
data clearly provides {\em some} information about the future.
...
is the second half. Let $\bar{y}$ and $\bar{z}$ be the averages of the two
subsets of the data:
\begin{align}
\bar{y} &= \frac{1}{50} \sum_{i=1}^{50} y_i \\
\bar{z} &= \frac{1}{50} \sum_{i=1}^{50} z_i.
\end{align}
In this section we use the algorithm to answer the questions:
i) How uncertain is $\bar{y}$?; ii) How uncertain is $\bar{z}$; and
iii) How informative is $\bar{y}$ about $\bar{z}$? These are quantified by
$H(\bar{y})$, $H(\bar{z})$, and $I(\bar{y}; \bar{z})$ respectively. This can
...
algorithm in terms of $\ln \bar{y}$ and $\ln \bar{z}$. This affects the
differential entropies but not the mutual information (which is invariant 
under changes of parameterisation).

To compute the entropy of $\ln \bar{y}$, the distance function was just the
absolute value of the distance between the NS particles and the
reference particle in terms of $\ln \bar{y}$.
